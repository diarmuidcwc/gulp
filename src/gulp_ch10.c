...
void *Reader(void *arg)
{
	...
	else
	{
		...
		if (!strcmp(dev, "-"))
		{
			...
			int sfd = -2;
			if (handle)
				sfd = pcap_get_selectable_fd(handle);
...
void *Writer(void *arg)
{
	...
	while (!done)
	{
		...
		if (n < writesize)
			writesize = n; /* write remainder next loop */
		if (check_block)
		{
			/*
...
#endif /* JUSTCOPY */
			if (ps_stat_len >= V_WIDTH)
			{ /* put stats in arg list */
